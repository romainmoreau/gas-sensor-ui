...
export class GasSensingUpdateService {
  ...
  getRanges(): Observable<GasSensingUpdatesRange[]> {
    return this.httpClient.get<GasSensingUpdatesRange[]>(
      `${this.urlPrefix}/ranges`
    );
  }
  ...
  ): Observable<GasSensingUpdates> {
    return this.httpClient.get<GasSensingUpdates>(
      `${
        this.urlPrefix
      }/updates/${sensorName}/${description}/${this.formatMoment(
        beginning
      )}/${this.formatMoment(end)}`,
  ...
  addOrUpdatePoint(
    ...
    let iExisting: number | undefined;
    let iBefore: number | undefined;
  ...
  normalizeDatas(datas: [number, number][][]): [number, number][][] {
    ...
    const normalizedDatas = datas.map(() => [] as [number, number][]);
    const indexes = datas.map(() => 0);
    while (!indexes.every((index, i) => index === datas[i].length)) {
      const iXMinSorted = indexes
        .map((index, i) => [index, i])
        .filter(([index, i]) => index < datas[i].length)
        .map(([index, i]) => [i, datas[i][index][0]])
        .sort((a, b) => a[1] - b[1]);
      const xMin = iXMinSorted[0][1];
      const iXMin = iXMinSorted
        .filter((iX) => iX[1] === xMin)
        .map((iX) => iX[0]);
      normalizedDatas.forEach((normalizedData, i) => {
        if (iXMin.includes(i)) {
          ...
          normalizedData.push([
            xMin,
            normalizedData[normalizedData.length - 1][1],
          ]);
        }
      });
    }
  ...
  sensorNamesGasSensingUpdatesToDatas(
    sensorNamesGasSensingUpdates: GasSensingUpdates[],
    beginning: moment.Moment
  ): [number, number][][] {
    return sensorNamesGasSensingUpdates.map((gasSensingUpdates) => {
      if (gasSensingUpdates.periodUpdates.length === 0) {
        if (gasSensingUpdates.firstOutOfPeriodUpdate) {
          return [
            this.gasSensingUpdateToData(
              gasSensingUpdates.firstOutOfPeriodUpdate,
              beginning
            ),
          ];
        } else {
        ...
        const firstPeriodMoment = this.parseMoment(
          firstPeriodUpdate.localDateTime
        );
        const periodDatas = gasSensingUpdates.periodUpdates.map(
          (gasSensingUpdate) => this.gasSensingUpdateToData(gasSensingUpdate)
        );
        if (
          firstPeriodMoment.isSame(beginning) ||
          !gasSensingUpdates.firstOutOfPeriodUpdate
        ) {
          return periodDatas;
        } else {
  ...
  gasSensingUpdateToData(
    gasSensingUpdate: GasSensingUpdate,
    m?: moment.Moment
  ): [number, number] {
    return [
  ...
  getIntervals(parameters: {
    description: string;
    unit: string;
  }): Observable<GasSensingInterval[]> {
